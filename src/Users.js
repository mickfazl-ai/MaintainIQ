...
const CSS = `
  ...
  .u-input:focus { border-color: #00ABE4; box-shadow: 0 0 0 3px rgba(0,171,228,0.12); }
  .u-input::placeholder { color: #b0c4d4; }
  .u-btn-primary {
    padding: 9px 20px; background: #00ABE4; color: #fff;
  ...
  .u-btn-ghost:hover { border-color: #00ABE4; color: #00ABE4; }
  .u-btn-danger {
    padding: 6px 13px; background: #fff; color: #dc2626;
  ...
  .u-role-select {
    ...
    border: 1.5px solid #d6e6f2; background: #fff; color: #1a2b3c;
    font-family: inherit; cursor: pointer; outline: none; transition: border-color 0.15s;
  }
  .u-role-select:focus { border-color: #00ABE4; }
`;

// ─── Role config ───────────────────────────────────────────────────────────────
const ROLE_COLOR = {
  admin:      ['#7c3aed', '#f5f3ff'],
  supervisor: ['#d97706', '#fef3c7'],
  technician: ['#00ABE4', '#e0f4ff'],
  operator:   ['#16a34a', '#dcfce7'],
};

function RoleBadge({ role }) {
  const [c, bg] = ROLE_COLOR[role] || ['#7a92a8', '#f1f5f9'];
  return (
      ...
      {toasts.map(t => {
        ...
        return (
          <div key={t.id} style={{
            ...
            <span style={{ fontSize:13, color:'var(--text-bright)', fontWeight:500 }}>{t.msg}</span>
          </div>
        );
      })}
    </div>
    ...
    <>
      ...
      <div style={{ animation:'fadeUp 0.4s ease both' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:28 }}>
          <div>
            <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:32, fontWeight:800, color:'var(--text-bright)', letterSpacing:'1px', textTransform:'uppercase', margin:0, lineHeight:1 }}>User Management</h2>
        ...
        {!loading && (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:24 }}>
            {[
              { label:'Total Users',  value:users.length,                                   color:'var(--cyan)', bg:'#e0f4ff' },
              { label:'Admins',       value:users.filter(u=>u.role==='admin').length,        color:'var(--purple)', bg:'#f5f3ff' },
              { label:'Supervisors',  value:users.filter(u=>u.role==='supervisor').length,   color:'var(--amber)', bg:'#fef3c7' },
              { label:'Technicians',  value:users.filter(u=>u.role==='technician'||u.role==='operator').length, color:'var(--green)', bg:'#dcfce7' },
            ].map((s, i) => (
              <div key={s.label} className="u-card" style={{ padding:'16px 20px', display:'flex', alignItems:'center', gap:14, opacity:0, animation:`fadeUp 0.4s ease ${i*60}ms forwards` }}>
                ...
                <span style={{ fontSize:12, fontWeight:600, color:'var(--text-mid)', lineHeight:1.3 }}>{s.label}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* ── Add user form ── */}
        {showForm && (
          <div className="u-card" style={{ marginBottom:20, borderLeft:'3px solid #00ABE4', animation:'fadeUp 0.3s ease' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <span style={{ fontSize:14, fontWeight:800, color:'var(--text-bright)' }}>Add New Team Member</span>
        ...
        <div className="u-card">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, paddingBottom:14, borderBottom:'1.5px solid #eaf3fb' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontSize:16, fontWeight:800, textTransform:'uppercase', letterSpacing:'0.8px', color:'var(--text-bright)' }}>Team Members</span>
              <span style={{ background:'#e0f4ff', color:'var(--cyan)', fontSize:11, fontWeight:700, padding:'2px 8px', borderRadius:20 }}>{users.length}</span>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign:'center', padding:'48px 20px' }}>
              ...
              <div style={{ fontSize:14, fontWeight:700, color:'var(--text-bright)', marginBottom:6 }}>No team members yet</div>
                  ...
                  <tr key={u.id} className="u-row" style={{ borderBottom:'1px solid #eaf3fb', opacity:0, animation:`fadeUp 0.3s ease ${i*40}ms forwards` }}>
                    <td style={{ padding:'13px 14px 13px 0' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <div>
                          <div style={{ fontSize:13, fontWeight:700, color:'var(--text-bright)' }}>{u.name || '—'}</div>
                          {u.email === userRole?.email && (
                            <span style={{ fontSize:10, color:'var(--cyan)', fontWeight:700, letterSpacing:'0.5px' }}>YOU</span>
                          )}
                        </div>
                      </div>
                    </td>
