...
function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    const pendingMaintenance = maintenance?.filter(m => m.status === 'Upcoming' || m.status === 'Due Soon').length || 0;
    const overduepm = maintenance?.filter(m => m.status === 'Overdue').length || 0;

    setStats({ totalAssets, machinesDown, pendingMaintenance, overduepm });
    ...
    <div className="dashboard">
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card down">
          <h3>Machines Down</h3>
          <p className="stat-number">{stats.machinesDown}</p>
        </div>
        ...
        <div className="stat-card overdue">
          <h3>Overdue PM's</h3>
          <p className="stat-number">{stats.overduepm}</p>
        </div>
      </div>
